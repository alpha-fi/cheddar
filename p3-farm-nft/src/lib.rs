...
use near_sdk::{
    assert_one_yocto, env, log, near_bindgen, AccountId, Balance, BorshStorageKey, PanicOnDefault, Promise,
...
impl Contract {
    ...
    pub fn unstake(&mut self, nft_contract_id: &AccountId, token_id: Option<TokenId>) -> Vec<String> {
        ...
        assert_one_yocto();
        let user = env::predecessor_account_id();
        self.internal_nft_unstake(&user, nft_contract_id, token_id).into()
    }
    ...
    fn transfer_staked_nft_token(&mut self, user: AccountId, nft_contract_i: usize, token_id: TokenId) -> Promise {
        // withdraw 1 token. Unimplemented fee
    ...
    fn transfer_farmed_tokens(&mut self, u: &AccountId, token_i: usize, amount: u128) -> Promise {
        ...
        println!("transfer farmed token: @{} ", token.clone());
    ...
    pub fn transfer_staked_callback(
        ...
        match env::promise_result(0) {
            ...
            PromiseResult::Successful(_) => {
                // self.fee_collected[nft_contract_i] += fee.0;

                log!("token withdrawn {}", token_id.clone());
            ...
            PromiseResult::Failed => {
                ...
                //self.total_stake[nft_contract_i] += full_amount;
                //recover only token, fee is unimplemented now
                self.recover_state(&user, true, nft_contract_i, Some(token_id), None);
            }
        }
    }
    ...
    pub fn transfer_farmed_callback(&mut self, user: AccountId, token_i: usize, amount: U128) {
        match env::promise_result(0) {
            ...
            PromiseResult::Failed => {
                ...
                self.recover_state(&user, false, token_i, None, Some(amount.0));
            }
        }
    }
        ...
        match env::promise_result(0) {
            ...
            PromiseResult::Failed => {
                ...
                if let Some(v2) = self.vaults.get(&user) {
                    ...
                    v = Vault::new(self.stake_nft_tokens.len(), self.reward_acc)
                }
                v.cheddy = cheddy;
    ...
    fn recover_state(
        ...
        // FT contract id
            ...
            self.total_harvested[contract_i] -= amount.unwrap();
        }
    ...
    fn create_account(&mut self, user: &AccountId) {
        self.vaults
            .insert(&user, &Vault::new(self.stake_nft_tokens.len(), self.reward_acc));
        self.accounts_registered += 1;
    }
...
mod tests {
    ...
    fn test_stake_unstake() {
        ...
        println!("user_1 status {:?}", ctr.status(user_1.clone()));
    ...
    #[test]
    fn test_stake_by_token_id_untake_all() {
        let user_1: AccountId = acc_u1();
        let nft1: AccountId = acc_staking1();
